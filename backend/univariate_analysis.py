...
for i, r in top_aset.iterrows(): print(f"- {r['Emiten']}: {r['Count']} tahun outliers survei ke atas")

print("\n=== OUTLIER ROA BAWAH (RUGI EKSTREM) ===")
for i, r in roa_out_low.sort_values('Value').head(10).iterrows(): print(f"- {r['Emiten']} ({r['Tahun']}): {r['Value']:,.2f}%")

print("\n=== OUTLIER ROA ATAS (UNTUNG EKSTREM) ===")
for i, r in roa_out_up.sort_values('Value', ascending=False).head(10).iterrows(): print(f"- {r['Emiten']} ({r['Tahun']}): {r['Value']:,.2f}%")
