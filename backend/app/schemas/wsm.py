...
class WSMMultiYearScoreRequest(BaseModel):
    template_id: Optional[int] = None
    weight_template_id: Optional[int] = Field(default=None, description="Optional weight template to apply")
    start_year: int
    end_year: int
...
class WSMMultiYearRankingItem(BaseModel):
    rank: int
    ticker: str
    average_score: float
    yearly_breakdown: List[YearlyScore]


class WSMMultiYearScoreResponse(BaseModel):
    start_year: int
    end_year: int
    missing_policy: Literal["redistribute", "zero", "drop"]
